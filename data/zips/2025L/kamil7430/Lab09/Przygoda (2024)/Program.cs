...
namespace ASD
{
    ...
    internal abstract class Lab10StageBase : TestCase
    {
        ...
        protected Graph originalLabyrinth;
        ...
        public Lab10StageBase(Graph labyrinth, int startingTorches, int debt, int[] roomTorches, int[] roomGold, bool expectedResult, double timeLimit, string description) : base(timeLimit, null, description)
        {
            ...
            originalLabyrinth = new Graph(labyrinth);
        }
    ...
    internal class Lab10Stage1Case : Lab10StageBase
    {
        ...
        protected override (Result resultCode, string message) VerifyTestCase(object settings)
        {
            ...
            int lastRoom = -1;
            bool[] visited = new bool[originalLabyrinth.VertexCount];
            int currentGold = 0;
            int currentTorches = startingTorches + 1;
            int exit = originalLabyrinth.VertexCount - 1;
            ...
            {
                if (room < 0 || room >= originalLabyrinth.VertexCount)
                {
                    return (Result.WrongResult, "Niepoprawny numer pokoju w drodze ucieczki");
                }

                if (lastRoom != -1 && !originalLabyrinth.HasEdge(lastRoom, room))
                {
    ...
    internal class Lab10Stage2Case : Lab10StageBase
    {
        ...
        protected override (Result resultCode, string message) VerifyTestCase(object settings)
        {
            ...
            int[] lastDistance = new int[originalLabyrinth.VertexCount];
            for (int i = 0; i < originalLabyrinth.VertexCount; i++) lastDistance[i] = -1;
            bool[] destroyed = new bool[originalLabyrinth.VertexCount];
            ...
            foreach (int room in route)
            {
                ...
                lastDistance[room] = currentDistance;
                if (currentDistance == dragonDelay + 1)
                {
                    dragon = 0;
                    destroyed[dragon] = true;
                }
                else if (currentDistance > dragonDelay + 1)
                {
                    dragon = route[lastDistance[dragon] + 1];
                    destroyed[dragon] = true;
                }
                currentDistance++;
    ...
    class Lab10Tests : TestModule
    {
        ...
        void PrepareTests()
        {
            ...
            //    stage2Tests.TestCases.Add(new Lab10Stage2Case(lab, 1, n - 2 + (n - 2) / 2, torches, gold, int.MaxValue / 2, true, 1, description));
            //}
        }
    }
